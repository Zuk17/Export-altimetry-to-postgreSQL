   ...
   m_pNoradModel(NULL)
{
   ...
   m_kmPerigeeRec       = XKMPER * (m_aeAxisSemiMinorRec * (1.0 - e) - AE);
...
double cOrbit::TPlusEpoch(time_t gmt) const
{
   return difftime(gmt, EpochTime());
}

//////////////////////////////////////////////////////////////////////////////
// Returns elapsed number of seconds from epoch to current time.
// Note: "Predicted" TLEs can have epochs in the future.
double cOrbit::TPlusEpoch() const
{
   return TPlusEpoch(time(NULL));
}

//////////////////////////////////////////////////////////////////////////////
// Returns the mean anomaly in radians at given GMT.
// At epoch, the mean anomaly is given by the elements data.
double cOrbit::mnAnomaly(time_t gmt) const
{
