...
void cNoradBase::Initialize()
{
   ...
   m_perigee = XKMPER * (m_aodp * (1.0 - m_satEcc) - AE);
   ...
   if (m_perigee < 156.0)
   {
      ...
      m_qoms24 = pow((120.0 - m_s4) * AE / XKMPER, 4.0);
      m_s4 = m_s4 / XKMPER + AE;
   }
                               ...
                               cEci &eci)
{
   ...
   double altKm = (vecPos.Magnitude() * (XKMPER / AE));

   if ((altKm < EARTH_RAD) || (altKm > (2 * GEOSYNC_ALT)))
      return false;
   ...
   time_t gmt = m_Orbit.EpochTime() + (time_t)(tsince * 60.0);

   eci = cEci(vecPos, vecVel, cJulian(gmt));
